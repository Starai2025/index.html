# JOHNATHAN RIGHT
123 Peachtree Street NE, Atlanta, GA 30303 | 404-419-7440 | johnathan.right@example.com | www.linkedin.com/in/starr-butts-664b98154

## PARTNER SUPPORT SPECIALIST | AI SAAS PARTNER OPERATIONS | B2B CUSTOMER OPERATIONS

B2B operations and AI SaaS sales professional with 7+ years of experience managing high-volume customer and partner-facing workflows across technology, education, healthcare, real estate, and program operations. Skilled in queue triage, CRM hygiene, escalation routing, written documentation, partner/account research, root-cause analysis, and process improvement. Known for turning messy account signals, support requests, and CRM data into clear workflows, reusable documentation, and fast next steps for internal teams and external stakeholders.

## CORE SKILLS

Partner & Customer Support | Ticket / Queue Triage | SLA-Equivalent Response Cadences | Partner Onboarding Support | Escalation Routing | Knowledge Base & FAQ Documentation | Root-Cause Analysis | Process Improvement & Automation | CRM Data Quality | Cross-Functional Collaboration | AI SaaS Prospecting | Salesforce | HubSpot | LinkedIn Sales Navigator | Apollo.io | ZoomInfo | Outreach.io | Salesloft | Evaboot | n8n | ChatGPT

## WORK EXPERIENCE

### Northstar Systems Group | Remote | 05/2026 - Present
**Partner Operations Specialist, AI SaaS**

- Supported [redacted] outbound and partner-facing workflows for high-value technical accounts, translating account [redacted] from operational leaders, implementation stakeholders, and buying committees into clear [redacted] actions, escalation notes, and next-step recommendations.
- Built a production-ready CRM workflow plan for outbound operations, including webhook intake, automation routing logic, custom properties, dedupe keys, contact/company upsert rules, and association checks.
- Created repeatable documentation for CRM sync setup, partner/account targeting, persona segmentation, campaign naming, and live-test verification, helping reduce repeated internal questions and improve self-service readiness.
- Identified recurring data, access, and entitlement risks across prospect imports, including missing emails, inconsistent company domains, duplicate record risk, payload variability, and permission dependencies; documented mitigations before scale.

### Meridian Education Partners | Remote | 08/2024 - Present
**Business Development Specialist, Education Solutions**

- Managed 80+ daily outbound and follow-up workflows across calls, email, LinkedIn, SMS, and CRM tasks while maintaining tight response cadences and clean account handoffs with senior sales partners.
- Booked 10-13 qualified meetings per month with executive and district-level stakeholders by diagnosing needs, documenting account context, and routing qualified opportunities to the right internal owners.
- Built a $250K+ outbound pipeline in 9 months by identifying recurring buyer pain points, improving message quality, and maintaining consistent CRM updates across active prospects.
- Applied AI-driven lead scoring to improve conversion rates to 20%, shorten the sales cycle to 12 days, and prioritize high-fit opportunities for cross-functional follow-up.

### Atlas Health Talent | Atlanta, GA | 10/2021 - 08/2024
**Client Development Representative, Healthcare Services**

- Achieved 105-115% quota per quarter while booking 12-14 discovery calls per month and supporting 3-4 monthly contract wins through high-volume account management.
- Executed 40-50 multi-channel outreach activities daily, using CRM notes, cadence performance, and A/B testing to identify patterns and improve qualification quality.
- Used AI-supported research, market intelligence tools, and sales engagement platforms to automate prospecting workflows, increasing outbound conversions by 9% and shortening sales cycles by 12 days.
- Partnered with internal sales and recruiting teams to resolve account questions, clarify stakeholder needs, and keep opportunities moving through the correct workflow.

### Peachtree Commercial Properties | Hybrid | 11/2019 - 10/2021
**Commercial Account Executive**

- Completed Sales Development and Account Executive training focused on discovery, buyer personas, presentation delivery, pain-point analysis, and consultative communication.
- Executed 150+ strategic outbound calls using Salesforce CRM to engage prospects, qualify opportunities, document needs, and expand pipeline.
- Closed 8-10 deals per month, grew accounts 15% year over year, and increased leased revenue by 39% in the first year through stronger account follow-up and buyer alignment.
- Used analytics to refine pricing, improve positioning, and lift close rates by 12-15% within one year.

### Global Student Pathways | Multi-State | 01/2017 - 11/2019
**Program Operations Manager**

- Directed program rollouts across 10 states while coordinating educators, families, school leaders, and internal teams through complex support and implementation workflows.
- Trained 150+ educators annually and created repeatable guidance for program adoption, stakeholder communication, and issue resolution.
- Expanded pipeline by 25% through 30+ district partnerships, strengthening long-term relationships and improving program visibility.
- Managed logistics for 300+ student placements annually, improving adoption by 12% through structured follow-up and cross-functional coordination.

## EDUCATION

**Bachelor of Science in Social Work**  
Rutgers University-Newark
